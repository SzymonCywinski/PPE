...
Przedmiotem prognozy jest miesięczna wielkość skupu mleka. Źródłem danych
...
Plik `mleko.csv` zawiera miesięczne dane nt skupu mleka w Polsce w okresie
styczeń 2010--październik 2020 (tabela 47: skup ważniejszych produktów rolnych; https://stat.gov.pl/obszary-tematyczne/inne-opracowania/) 
...
Wielkość skup mleka spada zimą a jest najwyższa w miesiącach maj--lipiec. Ostatni wykres
pokazuje wyraźnie, że sezonowość
skupu ma charakter addytywny (stała amplituda wahań) a trend jest zbliżony do liniowego
...
W analizowanym okresie średni skup mleka wynosił `r t.mean` (wartość minimalna `r t.min`; wartość
maksymalna `r t.max`)
...
jest to spowodowane nadzwyczajnym spadkiem skupu związanym z epidemię COVID19.
